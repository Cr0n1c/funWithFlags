...
function powerOn(playSound = true) {
    const sndOn = document.getElementById("snd_power_on");
    if (playSound && sndOn)
        sndOn.play().catch(e => console.warn("Power on sound blocked:", e));
    $("#switch").prop("checked", true);
    $(".surround").addClass("on");
    createCookie('power', '1');
}
function powerOff(playSound = true) {
    const sndOff = document.getElementById("snd_power_off");
    if (playSound && sndOff)
        sndOff.play().catch(e => console.warn("Power off sound blocked:", e));
